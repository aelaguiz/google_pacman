...
Actor.prototype = {
	...
	a: function (b) {
		...
		switch (c) {
			case 32:
				...
				break;
			case 8:
				if(g.ghostEyesCount > 0) {
					g.ghostEyesCount--;
				}
				else if(g.ghostEyesCount === 0) {
					g.playAmbientSound();
				}
				break;
		}
	...
	p: function (b) {
		...
		if(!this.ghost && g.playfield[b[0]][b[1]].dot) {
	...
	n: function () {
		...
		if(this.mode === 8 && this.pos[0] === s[0] && this.pos[1] === s[1]) {
			this.a(64);
		}
	...
	o: function () {
		...
		if(!this.ghost && this.nextDir && g.playfield[d[0]][d[1]].intersection && this.nextDir & g.playfield[d[0]][d[1]].allowedDir) {
			this.t();
		}
	},
	B: function () {
